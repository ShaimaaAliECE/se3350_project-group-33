...
//Build Array display with input from react component props
  ...
  for(let i =  0; i < buildArray.length; i++){
    ...
    if(typeof(buildArray[i]) === 'string'){
      let tempLength = (buildArray[i].length > 1) ? parseInt(buildArray[i].slice(1, buildArray[i].length)) : 1;
      totalDisplayCounter += tempLength;
    }
  }

  function isRight(){
    if(totalArrayCounter <= tutorialNumbers.length/2 && totalDisplayCounter >= 15){
        return  true;
    }
    else{
        return false;
